...
<style>
    ...
    #tabelPengembalian_wrapper .dataTables_length select,
    #tabelPengembalian_wrapper .dataTables_filter input {
    ...
    #tabelPengembalian_wrapper .dataTables_filter input:focus {
        box-shadow: 0 0 0 3px rgba(99,102,241,.2);
        border-color: #3b82f6;
    }
    #tabelPengembalian_wrapper .dataTables_paginate .paginate_button:hover:not(.current) {
    ...
    #tabelPengembalian_wrapper { max-width: 100%; width: 100%; }
    #tabelPengembalian_wrapper table.dataTable { width: 100% !important; }
    ...
    @media (max-width: 480px) {
        #tabelPengembalian_wrapper .dt-toolbar .dataTables_filter input { width: 90px; font-size: 0.7rem !important; padding: 0.2rem 0.4rem !important; }
        #tabelPengembalian_wrapper .dt-toolbar .dataTables_length select { padding: 0.2rem 1.25rem 0.2rem 0.35rem !important; font-size: 0.7rem !important; }
    ...
    @media (max-width: 370px) {
        #tabelPengembalian_wrapper .dt-toolbar .dataTables_length select,
        #tabelPengembalian_wrapper .dt-toolbar .dataTables_filter input,
        .dt-toolbar .dt-buttons-wrap a,
        ...
        #tabelPengembalian_wrapper .dt-toolbar .dataTables_filter input { width: 60px !important; }
        #tabelPengembalian_wrapper .dt-toolbar .dataTables_length select { padding: 0.15rem 0.8rem 0.15rem 0.2rem !important; }
...
<div class="space-y-5">
    ...
    <div class="hidden md:grid grid-cols-2 md:grid-cols-4 gap-3">
        @foreach([
            ['icon'=>'fas fa-undo-alt',       'color'=>'blue',  'label'=>'Total Pengembalian','val'=>$totalPengembalian,'delay'=>'0s'],
            ['icon'=>'fas fa-check-circle',   'color'=>'emerald','label'=>'Tepat Waktu',      'val'=>$tepatWaktu,     'delay'=>'.05s'],
            ['icon'=>'fas fa-exclamation-triangle','color'=>'rose','label'=>'Terlambat',       'val'=>$terlambatKembali,'delay'=>'.1s'],
            ['icon'=>'fas fa-money-bill-wave','color'=>'amber','label'=>'Total Denda',        'val'=>'Rp '.number_format($totalDenda,0,',','.'),'delay'=>'.15s'],
        ] as $card)
    ...
    <div id="tableActions" class="hidden flex flex-wrap items-center gap-2">
        <button onclick="openFilterModal()"
                class="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium border transition-all
                       {{ request()->hasAny(['tanggal_mulai','tanggal_akhir'])
                          ? 'bg-blue-600 text-white border-blue-600 shadow-md shadow-blue-200'
                          : 'bg-white text-gray-700 border-gray-200 hover:border-blue-400 hover:text-blue-600' }}">
            <i class="fas fa-sliders-h"></i>
            Filter
            @if(request()->hasAny(['tanggal_mulai','tanggal_akhir']))
            <span class="w-5 h-5 rounded-full bg-white/30 text-xs font-bold flex items-center justify-center">
                {{ collect(['tanggal_mulai','tanggal_akhir'])->filter(fn($k)=>request()->filled($k))->count() }}
            </span>
            @endif
        </button>
    ...
    <div class="glass-card rounded-2xl shadow-lg overflow-hidden animate-fade" style="animation-delay:.3s">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-gray-800 text-sm">Data Pengembalian</span>
            </div>
            <span class="text-xs text-gray-400">{{ now()->format('d M Y') }}</span>
        </div>

        <div class="p-4">
            <table id="tabelPengembalian" class="w-full no-footer" style="width:100%">
                <thead>
                    <tr class="bg-gradient-to-r from-blue-50 to-blue-100">
                        ...
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">No. Pengembalian</th>
                        ...
                        <th class="px-3 py-3 text-right text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Denda</th>
                    </tr>
                </thead>
    ...
    <div id="filterModalBox"
        ...
        <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-white">Filter Laporan Pengembalian</h3>
        ...
        <form method="GET" action="{{ route('admin.laporan.pengembalian') }}" id="filterForm" data-spa-ignore>
            <div class="px-6 py-5 space-y-4">

                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-calendar-alt text-blue-500 mr-1"></i> Rentang Tanggal Kembali
                    </label>
...
$(document).ready(function () {
    $('#tabelPengembalian').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.laporan.pengembalian") }}',
            type: 'GET',
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'nomor_pengembalian', name: 'nomor_pengembalian' },
            { data: 'anggota_info', name: 'anggota_id' },
            { data: 'tanggal_pengembalian', name: 'tanggal_pengembalian' },
            { data: 'jumlah_buku', name: 'jumlah_hari_terlambat' },
            { data: 'status_label', name: 'jumlah_hari_terlambat' },
        ...
        drawCallback: function () {
            ...
            $('#tabelPengembalian_wrapper').find('.paginate_button')
                .not('.previous, .next')
                .each(function () {
