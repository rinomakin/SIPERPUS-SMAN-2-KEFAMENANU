...
<style>
    ...
    #tabelBuku_wrapper { max-width: 100%; width: 100%; }
    #tabelBuku_wrapper table.dataTable { width: 100% !important; }
    ...
    @media (max-width: 480px) {
        #tabelBuku_wrapper .dt-toolbar .dataTables_filter input { width: 90px; font-size: 0.7rem !important; padding: 0.2rem 0.4rem !important; }
        #tabelBuku_wrapper .dt-toolbar .dataTables_length select { padding: 0.2rem 1.25rem 0.2rem 0.35rem !important; font-size: 0.7rem !important; }
    ...
    @media (max-width: 370px) {
        #tabelBuku_wrapper .dt-toolbar .dataTables_length select,
        #tabelBuku_wrapper .dt-toolbar .dataTables_filter input,
        .dt-toolbar .dt-buttons-wrap a,
        ...
        #tabelBuku_wrapper .dt-toolbar .dataTables_filter input { width: 60px !important; }
        #tabelBuku_wrapper .dt-toolbar .dataTables_length select { padding: 0.15rem 0.8rem 0.15rem 0.2rem !important; }
...
<div class="space-y-5">
    ...
    <div class="hidden md:grid grid-cols-2 md:grid-cols-4 gap-3">
        @foreach([
            ['icon'=>'fas fa-book',         'color'=>'blue',  'label'=>'Total Buku',     'val'=>$totalBuku,     'delay'=>'0s'],
            ['icon'=>'fas fa-layer-group',   'color'=>'blue',  'label'=>'Total Eksemplar','val'=>$totalEksemplar,'delay'=>'.05s'],
            ...
            <table id="tabelBuku" class="w-full no-footer" style="width:100%">
                <thead>
                    <tr class="bg-gradient-to-r from-blue-50 to-blue-100">
                        ...
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Judul Buku</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">ISBN</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Pengarang</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Kategori</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Jenis</th>
                        <th class="px-3 py-3 text-center text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Total Eks.</th>
                        <th class="px-3 py-3 text-center text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Tersedia</th>
...
<div id="filterModal"
    ...
    <div id="filterModalBox"
        ...
        <form method="GET" action="{{ route('admin.laporan.buku') }}" id="filterForm" data-spa-ignore>
            <div class="px-6 py-5 space-y-4">

                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-calendar-alt text-blue-500 mr-1"></i> Rentang Tanggal Input
                    </label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-tag text-blue-500 mr-1"></i> Kategori Buku
                    </label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-bookmark text-blue-500 mr-1"></i> Jenis Buku
                    </label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-boxes text-blue-500 mr-1"></i> Status Stok
                    </label>
...
$(document).ready(function () {
    $('#tabelBuku').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("admin.laporan.buku") }}',
            type: 'GET',
            data: function(d) {
                var params = new URLSearchParams(window.location.search);
                d.tanggal_mulai = params.get('tanggal_mulai') || '';
                d.tanggal_akhir = params.get('tanggal_akhir') || '';
                d.kategori_id = params.get('kategori_id') || '';
                d.jenis_buku_id = params.get('jenis_buku_id') || '';
        ...
        columns: [
            ...
            { data: 'kategori_label', name: 'kategori_id' },
            { data: 'jenis_label', name: 'jenis_buku_id' },
        ...
        drawCallback: function () {
            ...
            $('#tabelBuku_wrapper').find('.paginate_button')
                .not('.previous, .next')
                .each(function () {
