...
<style>
    ...
    #tabelDenda_wrapper .dataTables_length select,
    #tabelDenda_wrapper .dataTables_filter input {
    ...
    #tabelDenda_wrapper .dataTables_filter input:focus {
        box-shadow: 0 0 0 3px rgba(99,102,241,.2);
        border-color: #3b82f6;
    }
    #tabelDenda_wrapper .dataTables_paginate .paginate_button:hover:not(.current) {
    ...
    #tabelDenda_wrapper { max-width: 100%; width: 100%; }
    #tabelDenda_wrapper table.dataTable { width: 100% !important; }
    ...
    @media (max-width: 480px) {
        #tabelDenda_wrapper .dt-toolbar .dataTables_filter input { width: 90px; font-size: 0.7rem !important; padding: 0.2rem 0.4rem !important; }
        #tabelDenda_wrapper .dt-toolbar .dataTables_length select { padding: 0.2rem 1.25rem 0.2rem 0.35rem !important; font-size: 0.7rem !important; }
    ...
    @media (max-width: 370px) {
        #tabelDenda_wrapper .dt-toolbar .dataTables_length select,
        #tabelDenda_wrapper .dt-toolbar .dataTables_filter input,
        .dt-toolbar .dt-buttons-wrap a,
        ...
        #tabelDenda_wrapper .dt-toolbar .dataTables_filter input { width: 60px !important; }
        #tabelDenda_wrapper .dt-toolbar .dataTables_length select { padding: 0.15rem 0.8rem 0.15rem 0.2rem !important; }
...
<div class="space-y-5">
    ...
    <div class="hidden md:grid grid-cols-2 md:grid-cols-4 gap-3">
        @foreach([
            ['icon'=>'fas fa-money-bill-wave','color'=>'amber','label'=>'Total Denda',   'val'=>'Rp '.number_format($totalNominal,0,',','.'),'delay'=>'0s'],
            ['icon'=>'fas fa-exclamation-triangle','color'=>'rose','label'=>'Total Transaksi','val'=>$totalDenda,  'delay'=>'.05s'],
            ['icon'=>'fas fa-check-circle',   'color'=>'emerald','label'=>'Sudah Dibayar','val'=>$sudahDibayar,  'delay'=>'.1s'],
            ['icon'=>'fas fa-times-circle',   'color'=>'rose',  'label'=>'Belum Dibayar','val'=>$belumDibayar,   'delay'=>'.15s'],
        ] as $card)
    ...
    <div class="glass-card rounded-2xl shadow-lg overflow-hidden animate-fade" style="animation-delay:.3s">
        <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <span class="font-semibold text-gray-800 text-sm">Data Denda</span>
            </div>
            <span class="text-xs text-gray-400">{{ now()->format('d M Y') }}</span>
        </div>

        <div class="p-4">
            <table id="tabelDenda" class="w-full no-footer" style="width:100%">
                <thead>
                    <tr class="bg-gradient-to-r from-blue-50 to-blue-100">
                        ...
                        <th class="px-3 py-3 text-center text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Hari Terlambat</th>
                        <th class="px-3 py-3 text-right text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Jumlah Denda</th>
                        <th class="px-3 py-3 text-center text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Status</th>
                        <th class="px-3 py-3 text-left text-xs font-semibold text-blue-700 uppercase tracking-wide whitespace-nowrap">Tgl Bayar</th>
                    </tr>
                </thead>
    ...
    <div id="filterModalBox"
        ...
        <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-700">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-base font-bold text-white">Filter Laporan Denda</h3>
        ...
        <form method="GET" action="{{ route('admin.laporan.denda') }}" id="filterForm" data-spa-ignore>
            <div class="px-6 py-5 space-y-4">

                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-calendar-alt text-blue-500 mr-1"></i> Rentang Tanggal
                    </label>
                ...
                <div>
                    <label class="block text-xs font-semibold text-gray-900 mb-2">
                        <i class="fas fa-toggle-on text-blue-500 mr-1"></i> Status Pembayaran
                    </label>
                    <div class="grid grid-cols-3 gap-2">
                        @foreach(['' => 'Semua', 'sudah_dibayar' => 'Sudah Bayar', 'belum_dibayar' => 'Belum Bayar'] as $val => $label)
...
$(document).ready(function () {
    $('#tabelDenda').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: '{{ route("admin.laporan.denda") }}',
            type: 'GET',
            data: function(d) {
        ...
        columns: [
            ...
            { data: 'nomor_peminjaman', name: 'peminjaman_id' },
            { data: 'jumlah_hari_terlambat', name: 'jumlah_hari_terlambat' },
            { data: 'jumlah_denda', name: 'jumlah_denda' },
        ...
        drawCallback: function () {
            ...
            $('#tabelDenda_wrapper').find('.paginate_button')
                .not('.previous, .next')
                .each(function () {
