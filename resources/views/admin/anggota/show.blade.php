...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="glass-card rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-in">
        ...
        <!-- Profile Info -->
        <div class="px-6 pb-6 -mt-14 relative">
            <div class="flex flex-col sm:flex-row sm:items-end gap-4">
                ...
                <div class="flex-1 sm:pb-1">
                    <h1 class="text-xl sm:text-2xl font-bold text-gray-900">{{ $anggota->nama_lengkap }}</h1>
                    <div class="flex flex-wrap items-center gap-2 mt-1.5">
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border
                        ...
                        @endphp
                        <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium {{ $jenisConfig[0] }} border">
                            <i class="fas {{ $jenisConfig[1] }} text-[10px]"></i>{{ ucfirst($anggota->jenis_anggota) }}
                        </span>
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium {{ $statusConfig[0] }} border">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div class="stat-card glass-card rounded-xl border border-gray-100 p-4 relative overflow-hidden">
            ...
            <div class="text-2xl font-bold text-gray-900">{{ $totalPeminjaman }}</div>
            <div class="text-xs text-gray-500 mt-0.5">Total Peminjaman</div>
        </div>
        ...
        <div class="stat-card glass-card rounded-xl border border-gray-100 p-4 relative overflow-hidden">
            ...
            <div class="text-2xl font-bold text-gray-900">{{ $sedangDipinjam }}</div>
            <div class="text-xs text-gray-500 mt-0.5">Sedang Dipinjam</div>
        </div>
        ...
        <div class="stat-card glass-card rounded-xl border border-gray-100 p-4 relative overflow-hidden">
            ...
            <div class="text-2xl font-bold text-gray-900">{{ $dikembalikan }}</div>
            <div class="text-xs text-gray-500 mt-0.5">Dikembalikan</div>
        </div>
        ...
        <div class="stat-card glass-card rounded-xl border border-gray-100 p-4 relative overflow-hidden">
            ...
            <div class="text-2xl font-bold {{ $dendaBelumBayar > 0 ? 'text-red-600' : 'text-gray-900' }}">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div class="glass-card rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-in" style="animation-delay: 0.1s;">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
                ...
                <h3 class="text-sm font-semibold text-gray-800 uppercase tracking-wide">Informasi Pribadi</h3>
            </div>
            <div class="px-5 py-2">
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-id-card text-gray-400 text-xs"></i> NIK</span>
                    <span class="info-value font-mono">{{ $anggota->nik }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-phone text-gray-400 text-xs"></i> Telepon</span>
                    <span class="info-value">{{ $anggota->nomor_telepon }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-envelope text-gray-400 text-xs"></i> Email</span>
                    <span class="info-value {{ $anggota->email ? '' : 'text-gray-400' }}">{{ $anggota->email ?: '-' }}</span>
                </div>
                @if($anggota->tanggal_lahir)
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-birthday-cake text-gray-400 text-xs"></i> Tanggal Lahir</span>
                    <span class="info-value">{{ $anggota->tanggal_lahir->format('d F Y') }}</span>
                </div>
                @endif
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-map-marker-alt text-gray-400 text-xs"></i> Alamat</span>
        ...
        <div class="glass-card rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-in" style="animation-delay: 0.15s;">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
                ...
                <h3 class="text-sm font-semibold text-gray-800 uppercase tracking-wide">Informasi Keanggotaan</h3>
            </div>
            <div class="px-5 py-2">
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-hashtag text-gray-400 text-xs"></i> Nomor Anggota</span>
                    <span class="info-value font-mono bg-gray-50 px-2 py-0.5 rounded-md text-xs">{{ $anggota->nomor_anggota }}</span>
                </div>
                @if($anggota->kelas)
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-school text-gray-400 text-xs"></i> Kelas</span>
                    <span class="info-value">{{ $anggota->kelas->nama_kelas }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-graduation-cap text-gray-400 text-xs"></i> Jurusan</span>
                    <span class="info-value">{{ $anggota->kelas->jurusan->nama_jurusan ?? '-' }}</span>
                </div>
                @endif
                @if($anggota->jabatan)
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-briefcase text-gray-400 text-xs"></i> Jabatan</span>
                    <span class="info-value">{{ $anggota->jabatan }}</span>
                </div>
                @endif
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-calendar-check text-gray-400 text-xs"></i> Bergabung</span>
                    <span class="info-value">{{ $anggota->tanggal_bergabung->format('d F Y') }}</span>
                </div>
                <div class="info-row">
                    <span class="info-label"><i class="fas fa-clock text-gray-400 text-xs"></i> Terakhir Diupdate</span>
                    <span class="info-value text-gray-400 text-xs">{{ $anggota->updated_at->format('d/m/Y H:i') }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="glass-card rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-in" style="animation-delay: 0.2s;">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
            ...
            <h3 class="text-sm font-semibold text-gray-800 uppercase tracking-wide">Barcode Anggota</h3>
        </div>
        <div class="p-5">
            <div class="barcode-box max-w-sm mx-auto">
                ...
                <div class="font-mono text-sm font-semibold text-gray-700 tracking-widest">{{ $anggota->barcode_anggota }}</div>
                <div class="text-[11px] text-gray-400 mt-1">Code 128</div>
            </div>
        </div>
    </div>
    ...
    <div class="glass-card rounded-2xl shadow-sm border border-gray-100 overflow-hidden fade-in" style="animation-delay: 0.25s;">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h3 class="text-sm font-semibold text-gray-800 uppercase tracking-wide">Riwayat Peminjaman Terbaru</h3>
            </div>
            <span class="text-xs text-gray-400">{{ $totalPeminjaman }} total</span>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50/80">
                        ...
                        <th class="px-5 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Tgl Pinjam</th>
                        <th class="px-5 py-3 text-left text-[11px] font-semibold text-gray-500 uppercase tracking-wider">Tgl Kembali</th>
                ...
                <tbody class="divide-y divide-gray-50">
                    ...
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        <td class="px-5 py-3 text-sm font-mono text-gray-600">#{{ $pinjam->id }}</td>
                        <td class="px-5 py-3 text-sm text-gray-600">{{ $pinjam->tanggal_peminjaman->format('d/m/Y') }}</td>
                        <td class="px-5 py-3 text-sm text-gray-600">{{ $pinjam->tanggal_harus_kembali->format('d/m/Y') }}</td>
                        <td class="px-5 py-3">
                            @php
                            ...
                            @endphp
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium {{ $pinjamStatusConfig[0] }} border">
        ...
        <div class="px-5 py-3 border-t border-gray-100 text-center">
            <span class="text-xs text-gray-400">Menampilkan 5 dari {{ $totalPeminjaman }} peminjaman</span>
        </div>
        @endif
    </div>
    @endif
