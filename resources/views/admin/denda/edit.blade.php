    ...
    <div class="glass-card rounded-2xl shadow-sm border border-gray-100 p-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-sm font-bold text-gray-900">Edit Denda</h1>
                    <p class="text-[11px] text-gray-400">Ubah data denda keterlambatan pengembalian buku</p>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('admin.denda.show', $denda->id) }}"
                   class="toolbar-btn bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-100">
    ...
    <form action="{{ route('admin.denda.update', $denda->id) }}" method="POST">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-5 gap-5">
            ...
            <div class="lg:col-span-3 glass-card rounded-2xl shadow-sm border border-gray-100 p-5">
                ...
                <div id="selectedPeminjamanCard" class="mt-4 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center">
                            <i class="fas fa-check text-emerald-600 text-xs"></i>
                        </div>
                        <div>
                            <p class="text-xs font-semibold text-emerald-800">Peminjaman Saat Ini</p>
                            <p id="selectedInfoText" class="text-[10px] text-emerald-600">
                                {{ $denda->peminjaman && $denda->peminjaman->anggota ? $denda->peminjaman->anggota->nama_lengkap : '-' }}
                                @if($denda->peminjaman)
                                    - {{ $denda->peminjaman->tanggal_harus_kembali }}
                                @endif
                            </p>
                        </div>
                    </div>
                    <input type="hidden" name="peminjaman_id" id="peminjaman_id" value="{{ $denda->peminjaman_id }}">
                </div>
            </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="catatan" name="catatan" rows="3"
                                  ...
                                  class="w-full px-4 py-3 text-xs bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500/20 focus:border-red-400 transition-all">{{ old('catatan', $denda->catatan) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
...
@if($denda->peminjaman)
    @php
        $p = $denda->peminjaman;
        $hariTerlambat = $denda->jumlah_hari_terlambat;
    @endphp
    currentPeminjamanData = {
        id: {{ $p->id }},
        hari_terlambat: {{ $hariTerlambat }},
        denda: {{ $denda->jumlah_denda }}
    };
@endif
            ...
            if (item.type === 'unpaid_denda') {
                ...
                section.querySelectorAll('.search-result-denda').forEach(el => {
                    el.addEventListener('click', () => {
                        ...
                        if (infoText) {
                            infoText.textContent = `${anggotaNama} - Terlambat ${hariTerlambat} hari - Rp ${Number(dendaVal).toLocaleString('id-ID')}`;
                        }
                        currentPeminjamanData = { id: peminjamanId, hari_terlambat: hariTerlambat, denda: dendaVal };
                    });
                });
                return;
            }
...
function selectPeminjaman(card, pinjam) {
    ...
    if (!infoText) {
        ...
        newText.id = 'selectedInfoText';
        newText.className = 'text-[10px] text-emerald-600';
        infoCard.querySelector('.flex.items-center.gap-3 div').appendChild(newText);
    }
    ...
    currentPeminjamanData = { id: pinjam.id, hari_terlambat: pinjam.hari_terlambat, denda: pinjam.denda };
}
